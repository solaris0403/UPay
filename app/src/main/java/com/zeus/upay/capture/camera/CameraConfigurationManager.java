 ...
 */
package com.zeus.upay.capture.camera;
...
public final class CameraConfigurationManager {
	...
	public void initFromCameraParameters(Camera camera, SurfaceView surfaceView) {
		this.surfaceView = surfaceView;
		int w = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
		int h = View.MeasureSpec.makeMeasureSpec(0, View.MeasureSpec.UNSPECIFIED);
		surfaceView.measure(w, h);
		point = new Point(surfaceView.getMeasuredWidth(), surfaceView.getMeasuredHeight());

		Camera.Parameters parameters = camera.getParameters();
		if (point == null) {
			// 获取屏幕尺寸 默认全屏 如果是自定义的SurfaceView则需要自己计算
	...
	//	surfaceView.setLayoutParams(new RelativeLayout.LayoutParams(cameraResolution.x, cameraResolution.y));
//		LayoutParams layoutParams = surfaceView.getLayoutParams();
//		layoutParams.width = cameraResolution.x;
//		layoutParams.height = cameraResolution.y;
